# Dark Factory — Backlog

Единственный источник правды для описаний предстоящих задач.
Детали реализации, архитектурные решения, статус — всё здесь.

---

## Edit Mode

### [EDT-001] ✅ Clone card (без LLM)
**Реализован:** сессия 2026-05-11 #8
**Суть:** Clone — отдельная workspace-операция, не через Edit/delta-architect.
`POST /api/cards/:slug/clone` — `cp` + детерминированный find-and-replace трёх форм namespace (kebab/dot/slash) по всем текстовым файлам. Rename папки, обновление реестра.
UI: кнопка Clone на карточке в My Apps + модалка с полем new slug + валидация kebab-case.
SPA fix: `/my-apps` добавлен в Express SPA fallback (был 404 после рестарта).

**Решение по rename-via-Edit (оригинальная постановка):** отдельный rename через Edit нецелесообразен — LLM не справляется со сквозным переименованием. Clone покрывает нужду: скопировал → переименовал → доработал через Edit.

---

### [TPL-001] Проверить и исправить меню «три точки» в шаблонах
**Источник:** рабочий проект, сессия 2026-05-08
**Суть:** В рабочих карточках выполнена миграция: `ActionSheet` → `sap.m.Menu`,
добавлена кнопка «Required Authorization» (SDK `AuthorizationDialog`), переименованы обработчики,
`AboutDialog` переведён на `FormattedText`-паттерн.
Нужно проверить соответствие шаблонов DF и при расхождении — обновить по инструкции.
**Инструкция:** [`docs/MORE_ACTIONS_MENU_MIGRATION.md`](MORE_ACTIONS_MENU_MIGRATION.md)
**Приоритет:** низкий — **после демо**

---

### [TPL-002] Автовывод slug Архитектором + конвенция namespace/папка
**Источник:** устное обсуждение, сессия 2026-05-09
**Суть:** При ручном изготовлении карточек название папки должно соответствовать namespace —
занимает много времени и почти всегда содержит ошибки. В CARDS.md нет единой конвенции.
**Что делать:**
1. Проанализировать CARDS.md — найти доминирующую тенденцию (namespace → slug)
2. Зафиксировать конвенцию в `docs/contracts.md` и системном промпте Архитектора
3. Если в заказе не указан slug — Архитектор выводит из описания, показывает в Spec Review;
   пользователь корректирует если нужно (не спрашивает явно)
**Приоритет:** средний — **до демо** (~2 ч)

---

### [TPL-004] protocol + viewControls как ортогональные измерения
**Источник:** стратегическое обсуждение 2026-05-10, финализирован 2026-05-10 сессия #5

**Архитектурное решение (финальное):**
- `spec.protocol` — `"rest"` | `"odata2"` | `"odata4"` | `"other"` — **всегда спрашивать явно**, никогда не выводить автоматически (даже из JSON-sample)
- `spec.viewControls: string[]` — массив конкретных UI5-классов (`["sap.m.FilterBar", "sap.m.Table"]`) — выводить из мокапа/заказа, спрашивать если неясно
- `spec.layout` — **удалить** (заменён на viewControls)

**Два шаблона по протоколу (не по контролам):**

| Аспект | template-rest | template-odata2 |
|--------|--------------|-----------------|
| Загрузка данных | `oCard.request()` → `DataHelper._processData()` | OData2 model binding, `metadataLoaded()` → `_bindView()` |
| MockServer | `setRequests([{path, response}])` | `MockServer.simulate()` с `metadata.xml` + `EntitySet.json` |
| DataHelper | есть (`_processData` обязателен) | **нет вообще** |
| Extension point | `_processData()`, View FormElements | entity set name в `_bindView()`, Column definitions |

Шаблон — только стартовая файловая структура. Developer адаптирует содержимое под `spec.viewControls` + `spec.protocol` вне зависимости от шаблона.

**Порог сложности layout-а:**
- `viewControls.length <= 3` → Developer строит layout автоматически
- `viewControls.length > 3` → Архитектор показывает в Spec Review:
  > *"Layout содержит N контролов — сложная композиция. Рекомендую: сгенерировать скелет (основной контрол + структура), остальное доработать через вайб-кодинг — для сложных layout-ов это точнее и быстрее. Продолжить или упростить заказ?"*
- Условная видимость контролов (MessageStrip по состоянию и т.п.) — дополнительный сигнал сложности, учитывать наравне с количеством.

**Подход к реализации (упрощённый — версия 1):**
Не читать файлы с диска. Промпты LLM остаются inline T_*.

**Что делать:**
1. ~~Переименовать `templates/form-rest/` → `templates/template-rest/`, `templates/table-odata2/` → `templates/template-odata2/`~~ ✅ (сессия 2026-05-10 #4)
2. `template-odata2/`: заменить `_bindTable()` → `_bindView()` в `Main.controller.js`
3. `architect.js` — убрать `layout`, добавить `viewControls` (массив); protocol — всегда явный вопрос; предупреждение при > 3 контролах
4. `developer.js` — заменить все условия по `spec.layout` на `spec.viewControls`-based; убрать `isTable = spec.layout === 'table'`
5. `contracts.md` — обновить IC spec fields

**Приоритет:** высокий — **до демо** (~2-3 ч)

---

## UX

### [UX-006] Импорт папки (без архива)
**Источник:** стратегическое обсуждение 2026-05-10
**Суть:** Сейчас import принимает только .zip. Поддержать загрузку папки целиком через
`<input webkitdirectory>` — FileList с относительными путями. Сервер реконструирует структуру.
**Приоритет:** средний — **до демо** (~3 ч)

---

### [UX-007] ID карточек + ссылки в заказе ("как #32")
**Источник:** стратегическое обсуждение 2026-05-10
**Суть:**
- Каждой карточке присваивается auto-increment `id` в cards-registry.json
- В UI: бейджик `#28` на карточке + иконка копирования
- В заказе: `"сделай как #32"` → Архитектор получает spec.json референсной карточки как контекст
- Реализация: spec-only (3A) — инжектим только spec.json, не код файлов

**Что делать:**
1. `cards-registry.js` — добавить `id` при `registerCard()`
2. `client/app.js` — бейджик + copy button на карточке
3. `server/index.js` — при получении заказа парсить `#NNN`, загружать spec.json по id
4. `architect.js` — принимать `referenceSpecs[]` в userPrompt
**Приоритет:** высокий — **до демо** (~6 ч)

---

### [UX-010] Показывать выбранный файл/папку в диалоге импорта

**Источник:** устное обсуждение 2026-05-12
**Суть:** После выбора файла (.zip) или папки в диалоге Import Card показывать имя выбранного
файла/папки — сейчас после выбора UI не даёт никакой обратной связи.
**Приоритет:** средний — после демо (~1 ч)

---

### [UX-011] Импорт — стиль и спиннер

**Источник:** устное обсуждение 2026-05-13
**Суть:** Два улучшения диалога Import Card:
1. Поле «Local path on server» оформить в том же стиле, что ZIP и folder inputs (обёртка, лейбл, подсказка)
2. Во время импорта показывать спиннер (аналогично loading-overlay на главном экране)
**Приоритет:** средний — до демо (~1 ч)

---

### [UX-009] UI polish — финальный причёс перед демо
**Источник:** стратегическое обсуждение 2026-05-10
**Суть:** Все тексты интерфейса на английском, стиль выровнен под VSCode, мелкие UX-несоответствия.
**Приоритет:** высокий — **последний пункт до демо** (~2-3 ч)

---

### [UX-012] Hub tiles — layout 2×2
**Источник:** устное обсуждение 2026-05-12
**Суть:** Сейчас тайлы идут в ряд. Расположить по сетке 2×2.
**Приоритет:** средний — после демо (~30 мин)

---

### [UX-013] Chronicle tile — цвет
**Источник:** устное обсуждение 2026-05-12
**Суть:** Тайл Chronicle не отличается визуально от остальных. Выделить цветом или акцентом.
**Приоритет:** низкий — после демо (~30 мин)

---

### [UX-014] Текст тайлов — выравнивание и высота иконок
**Источник:** устное обсуждение 2026-05-12
**Суть:** Тексты тайлов должны занимать одинаковое количество строк (1 или 2),
иначе иконки оказываются на разной высоте. Нужен единый стандарт.
**Приоритет:** средний — после демо (~1 ч)

---

### [UX-015] Chronicle — стили диалога
**Источник:** устное обсуждение 2026-05-12
**Суть:** Облагородить стили Chronicle-диалога: отступы, типографика, визуальная иерархия.
**Приоритет:** низкий — после демо (~1 ч)

---

## Documentation

### [DOC-001] ✅ Chronicle — генерация changelog и обновление версий
**Реализован:** сессия 2026-05-12 #13
**Суть:** Тайл Chronicle в DF Hub. Автодетект ветки + последнего тега `slug@x.y.z` + коммитов с тега до HEAD.
LLM генерирует summary и changelogRow. Apply обновляет manifest.json, package.json, README.md (секция `## Version History`), confluence.md.
Версия предлагается автоматически: max(тег, manifest, package, README) + 1 по patch.
Теги именуются с префиксом slug: `employee-directory@0.0.1`.

---

### [DOC-002] README + confluence.md v1.0 при генерации карточки
**Источник:** устное обсуждение 2026-05-12
**Суть:** При первой генерации карточки автоматически создавать README.md и confluence.md с заголовком,
описанием и пустой секцией `## Version History` — чтобы Chronicle сразу находил нужную таблицу.
**Приоритет:** средний — после демо (~2 ч)

---

### [DOC-003] Chronicle — умный поиск таблицы версий в README
**Источник:** устное обсуждение 2026-05-12
**Суть:** Сейчас Chronicle ищет строго `## Version History`. Если в README другой заголовок
(`## Versions`, `## Changelog`, `## История версий`) — не находит и создаёт дублирующую секцию.
Задать несколько ключевых слов / регулярку для поиска существующей таблицы.
**Приоритет:** средний — после демо (~1 ч)

---

## Demo

### [DEMO-003] Onboarding-документ для тимлида и команды
**Источник:** устное обсуждение 2026-05-12
**Суть:** Одностраничный документ: что такое DF-IC, как включить в проект, что нужно для запуска.
Аудитория — тимлид (решение) + коллеги-разработчики (исполнение).
**Содержание:**
- Что такое DF и зачем он нужен (2-3 предложения)
- Что нужно: Node.js, ключ Hyperspace (к кому идти), `npm run start`
- Как настроить путь к карточкам проекта (`CARDS_DIR` → `../content/`)
- Как коллега заходит и работает
**Приоритет:** средний — после одобрения тимлидом

---

 demo/chronicle-source — три именованных тега
**Источник:** устное обсуждение 2026-05-12
**Суть:** Сейчас cherry-pick во время демо делается по хэшам (нечитаемо).
Расставить три тега на коммитах ветки `demo/chronicle-source`:
```
demo/fix-description
demo/ux-search
demo/feat-phone
```
Команда для демо станет читаемой:
```bash
git cherry-pick demo/fix-description demo/ux-search demo/feat-phone
```
**Приоритет:** высокий — до следующего прогона (~15 мин)

---

## Infrastructure

### [INF-002] ✅ Проактивный split Developer-вызова
**Реализован:** сессия 2026-05-10 #3 + bug fix в сессии #4
**Bug fix:** `viewGeneratorSystemPrompt` не попал в default export → View.view.xml не генерировался.

---

## Visualization

### [VIZ-001] ✅ Local sandbox для предпросмотра сгенерированной карточки
**Реализован:** сессия 2026-05-09 (v0.10)

---

## Info

### [INFO-001] Инфо-бунт — аудит и приведение документации в порядок

**Источник:** стратегическое обсуждение 2026-05-12
**Суть:** Привести все документы в соответствие с новым протоколом (PROTOCOL.md):
правильная температура, единственный источник правды, удалить дубли и устаревшее.
**Статус:** в работе

**Саб-таски:**
- [x] INFO-001-1 Инвентаризация — полный список всех docs/ и memory/ с предварительной оценкой
- [x] INFO-001-2 Memory-файлы — дубли, устаревшее, объединение (сессия #12)
- [ ] INFO-001-3 WARM-документы — backlog.md, DEMO.md актуальность
- [ ] INFO-001-4 COLD-документы — contracts.md, log.md, MORE_ACTIONS_MENU_MIGRATION.md
- [x] INFO-001-5 CONCEPT.md, ROADMAP.md — решение: оставить как COLD, добавить в CLAUDE.md (сессия #12)
- [x] INFO-001-6 Финал — MEMORY.md + CLAUDE.md обновлены (сессия #12)

**Приоритет:** параллельно с основной работой, по кусочкам

---



### [DOC-002] README + confluence.md с версией 1.0 при генерации карточки

**Источник:** устное обсуждение 2026-05-12
**Суть:** При создании новой карточки через DF-IC автоматически генерировать:
- `README.md` — с таблицей версий (версия 1.0, дата, «Initial generation via DF-IC»)
- `confluence.md` — то же самое в Confluence wiki-markup (`h1.`, `||` таблица)

Закрывает жизненный цикл: карточка создаётся уже с документационными заготовками,
которые Chronicle (DOC-001) потом дополняет.

**Приоритет:** после демо (~1 ч)

---

### [DOC-001] Chronicle — автообновление документации карточки

**Источник:** стратегическое обсуждение 2026-05-12
**Суть:** По коммитам между двумя тегами версий LLM резюмирует изменения и обновляет документацию:
- README: добавляет запись «версия N — что изменилось»
- Генерирует фрагмент для Confluence рядом с карточкой (copy-paste вручную, ~30 сек)

**Интерфейс:** два git-ref (`from`, `to`) → `git log from..to` → LLM summary → два файла.
Каждый ref — тег, хэш коммита или `HEAD`. Тег — рекомендованный способ версионирования,
но интерфейс не ограничен: `v17..v18`, `abc123..HEAD`, `v15..def456` — всё работает.

Реализация самодостаточная — без внешних API. Один LLM-вызов.

В демо: третий блок после Clone — «прошло время, внёс правки, теперь документируем».
Закрывает жизненный цикл: создал → переиспользовал → задокументировал.

**Тестирование и демо-сценарий:**
- Fixture (`demo/fixture-card/`) — база
- Ветка `demo/chronicle-test` — коммиты с правками + теги `v1`, `v2`
- Chronicle запускается на диапазоне тегов
- Ветку не удалять — нужна для повторных прогонов

**Приоритет:** до демо (~3-4 ч)

---

## Demo & Testing

### [DEMO-001] Fixture-карточка для демо и тестов

**Источник:** стратегическое обсуждение 2026-05-12
**Суть:** Готовая карточка с нейтральным содержимым, хранится в `demo/fixture-card/` в корне
проекта — вне workspace, не удалится случайно. Используется для:
- демо тимлиду: импортируем → клонируем → Chronicle
- регрессионных прогонов import/clone/Chronicle

**Название:** `employee-directory` (slug), namespace `com.sap.demo.employeedirectory`

**Подход:**
1. Сгенерировать через DF-IC (нейтральный заказ: «список сотрудников с фильтром по отделу», REST, sap.m.Table)
2. Вручную обогатить до уровня реальной карточки: Fragment-файлы, дополнительные хелперы — реальные карточки шире того, что генерирует DF-IC
3. Добавить несколько коммитов с правками — чтобы Chronicle было что показать
4. Зафиксировать путь в DEMO.md

**Приоритет:** до демо — нужна для всех трёх блоков

---

## Закрытые UX

### [UX-002] ✅ Режим редактирования (My Apps + Edit + Import) — v0.11
### [UX-003] ✅ Опция "Другое" в вопросах архитектора — v0.12.1
### [UX-004] ✅ My Apps — сортировка новые вверху — v0.12
### [UX-005] ✅ Сохранять spec.json в корне карточки — v0.12
### [UX-008] ✅ Загрузка мокапа + vision pre-pass — v0.13 (сессия 2026-05-10 #3/#4)
