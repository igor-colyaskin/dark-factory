...
### v0.2.1 — My Apps (single-user)

  Цель: владелец DF видит и управляет всеми своими приложениями.
  Контекст: кащейная стадия, хранилище без понятия сессий.

  [ ] Хранилище state/apps.json (единое для owner'а)
  [ ] Endpoints: GET /api/my-apps, POST /api/apps/:id/extend, DELETE /api/apps/:id
  [ ] UI блок "Мои приложения"
  [ ] Кнопки "Продлить на 24 часа", "Удалить сейчас"
  [ ] Автоматическое удаление по TTL (cleanup worker)
  [ ] Для v0.3 подготовить: возможность "бессрочного" приложения без TTL

### v0.3 — Services, not pages (Telegram focus)

  Цель: DF учится создавать автономные сервисы — приложения, которые
  продолжают работать без пользователя на странице.
  Флагманский класс: Telegram-каналы с автопубликацией.
  Подробности: docs/VISION_v0.3.md.

  [ ] v0.3.0 — Secrets & Scheduler infrastructure
  [ ] v0.3.1 — First Telegram Channel (demo-заказ)
  [ ] v0.3.2 — Polish на основе dogfooding

### v0.4+ — возможные направления

  После v0.3 и реального опыта использования. Пока — открытый список:
  - Feedback-loop между агентами (Tester → Developer с конкретным фидбэком)
  - Vision-модель для мокапов в блоке Order
  - Agregator-приложения (scraping + дайджесты)
  - Email / Discord / Slack интеграции
  - PWA для мобильного UX
  - Voice-интерфейсы (Web Speech API)
  - Каталог шаблонов в блоке Order
  - Выбор "качество vs стоимость" для моделей агентов
...
## 11. Требования к сгенерированному приложению — v0.2 base class
