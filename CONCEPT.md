...
### От велосипеда к авиазаводу

Dark Factory развивается по метафоре постепенного роста.

- **v0.1** — велосипед: pipeline работает, учебная задача решена.
- **v0.2** — скутер: приложения деплоятся, публичная ссылка.
- **v1.0** — зрелый агент: DF ведёт полноценные переговоры — спрашивает,
  предупреждает, проверяет, помнит.

**Стратегический горизонт: авиазавод.**

Не один самолёт, который становится лучше — а **платформа** (Proto DF),
из которой собираются специализированные фабрики под конкретные домены:

- **DF-Integration Card** — SAP Integration Cards из mockup + backend data
- **DF-SAP CAP UI5** — BE (SAP CAP) + FE (SAP UI5 freestyle) по спеке
- **DF-Telegram** — Telegram боты и каналы

Proto DF = авиазавод. Специализированная DF = конкретная модель самолёта.
Коллега берёт инстанс, настраивает под свой домен, использует.

### Оценка затрат: DF-base vs плагин

Эмпирическая оценка по DF-IC: **DF-base ≈ 30%, плагин ≈ 70%**.

- **База (30%):** оркестратор, agent-manager, file-manager, UI-оболочка — универсальны, не меняются между продуктами.
- **Плагин (70%):** промпты (architect, developer, tester) — это львиная доля работы. Плюс плагин-специфичный UI, verifier, routes, static generators.

База оправдывает себя при 2+ продуктах. При старте любой новой DF-XXX — первый вопрос: что берём из base, что пишем заново.

Каждая промежуточная версия — один новый цех завода. Не обязательно
прямой путь: возможны развилки, эксперименты, откаты. Важно движение.

### Почему это важно

В agency-векторе развития результаты каждой версии — **тонкие и концептуальные**
("архитектор теперь показывает spec до разработки"), а не осязаемые
("добавилась кнопка Download"). Метафора авиазавода — способ держать
долгосрочную цель в поле зрения: каждая версия — цех, который делает
следующую специализацию возможной.
...
- **Статус:** v0.13 — в работе (demo prep). Выпущены: v0.11, v0.11.1, v0.12, v0.12.1.
